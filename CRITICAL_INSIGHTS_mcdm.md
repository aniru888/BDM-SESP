...
This project requires application of MCDM (Multi-Criteria Decision Making) methods from the BDM course.

### When to Use Which Method

| Question Type | Use This | Why |
|---------------|----------|-----|
| "How important is Criterion A vs B?" | **AHP** | Need to derive weights from expert judgment |
| "Which alternative is best given criteria + data?" | **TOPSIS** | Have numerical data, need distance-based ranking |
| "How efficient is each option vs best performers?" | **DEA** | Comparing similar units on input-output efficiency |

---

### AHP (Analytic Hierarchy Process)

**Use when:** You need to weight criteria through subjective comparison.

**Key steps:**
1. Define hierarchy: Goal → Criteria → Alternatives
2. Pairwise compare criteria using Saaty scale (1-9)
3. Calculate priority weights (eigenvector)
4. Check consistency (CR < 0.1)
5. Compare alternatives on each criterion
6. Aggregate scores
...
If A vs B = 5, then B vs A = 1/5 (reciprocal)
```
...
RI values: n=3→0.58, n=4→0.90, n=5→1.12

CR < 0.1 → Proceed
CR ≥ 0.1 → Redo comparisons
```

**In SESP project:**
- Task 2.0.1: Weight criteria for incentive mechanism
- Task 5.0.1: Weight criteria for regional launch

---

### TOPSIS (Technique for Order Preference by Similarity to Ideal Solution)

**Use when:** You have numerical data and given/derived weights, need to rank alternatives.

**Key steps:**
1. Normalize decision matrix: `r_ij = x_ij / sqrt(sum(x_ij²))`
2. Apply weights: `v_ij = w_j × r_ij`
3. Find ideal (A*) and negative-ideal (A')
   - A* = {max for benefit criteria, min for cost criteria}
   - A' = {min for benefit criteria, max for cost criteria}
4. Calculate separation: `S* = sqrt(sum((v_ij - A*_j)²))`
5. Calculate closeness: `C* = S' / (S* + S')`
6. Rank by C* (higher = better)

**Interpretation:**
```
C* = 1 → At ideal solution
C* = 0 → At worst solution
Highest C* wins
```

**In SESP project:**
- Task 2.0.2: Rank pricing scenarios
- Task 5.0.2: Rank regional launch options

---

### DEA (Data Envelopment Analysis)

**Use when:** Comparing similar units on efficiency (who does more with less?).

**Key concepts:**
- DMUs = Decision Making Units (things being compared)
- Inputs = Resources consumed
- Outputs = Value produced
- Efficiency = Weighted outputs / Weighted inputs

**Orientation:**
- Input-oriented: "How much can we reduce inputs keeping output same?"
- Output-oriented: "How much can we increase output keeping inputs same?"

**Interpretation:**
```
Efficiency = 1.0 → On efficient frontier (benchmark)
Efficiency < 1.0 → Inefficient (needs improvement)

Example: Efficiency = 0.85 means 15% improvement possible
```

**In SESP project:**
- Task 2.0.3: Compare Light/Moderate/Heavy plan efficiency
...
```python
# AHP - Calculate weights from pairwise comparison matrix
import numpy as np
...
def topsis(decision_matrix, weights, criteria_types):
    """
    decision_matrix: numpy array (alternatives × criteria)
    ...
    ideal = np.where(criteria_types == 'benefit', weighted.max(axis=0), weighted.min(axis=0))
    neg_ideal = np.where(criteria_types == 'benefit', weighted.min(axis=0), weighted.max(axis=0))
    
    # Step 4: Separation measures
    s_pos = np.sqrt(((weighted - ideal)**2).sum(axis=1))
    s_neg = np.sqrt(((weighted - neg_ideal)**2).sum(axis=1))
    ...
    return {
        'closeness': closeness,
        'ranking': np.argsort(closeness)[::-1] + 1  # 1-indexed ranking
    }


# DEA - Basic efficiency (simplified)
from scipy.optimize import linprog

def dea_efficiency(inputs, outputs, dmu_index):
    """
    Calculate efficiency for one DMU using CRS input-oriented DEA.
    inputs: matrix (DMUs × input_count)
    outputs: matrix (DMUs × output_count)
    dmu_index: which DMU to evaluate
    """
    n_dmus, n_inputs = inputs.shape
    _, n_outputs = outputs.shape
    
    # Objective: minimize sum of input weights for target DMU
    # Subject to: efficiency <= 1 for all DMUs
    
    # This is simplified - full DEA requires proper LP formulation
    # For course project, can use scipy.optimize.linprog or PuLP
    
    pass  # Implement based on course LP formulation
```
...
| 2025-01-03 | v2.1: MCDM Methods Integration | BDM course requires AHP, TOPSIS, DEA application |
| | Added: AHP for incentive mechanism weights | Systematic criteria weighting |
| | Added: TOPSIS for pricing scenario ranking | Data-driven alternative ranking |
| | Added: DEA for plan efficiency analysis | Efficiency frontier identification |
| | Added: AHP + TOPSIS for regional launch | Regional selection decision support |
